use crate::commands::randomize::escape_mentions;
...
pub async fn history(
    ...
    let lines: Vec<String> = entries
        .iter()
        .map(|e| {
            let old_raw = e.old_nick.as_deref().unwrap_or(&e.user_name);
            let old = escape_mentions(old_raw);
            let new_nick = escape_mentions(&e.new_nick);
            format!(
                "`{}` **{}** → **{}** *({})*",
